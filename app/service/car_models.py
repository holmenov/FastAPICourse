...
class CarModelsService(BaseService):
    async def get_filtered_by_time(
        self, pagination, mark_name, car_model_name, car_model_year, date_from, date_to
    ):
    ...
    async def add_car(
        ...
        if car_data.features:
            features = [
                SCarsFeaturesData(car_id=added_car.id, feature_id=f_id)
                for f_id in car_data.features
            ]
    ...
    async def partially_edit_car(
        self, mark_name: str, car_id: int, car_data: SCarModelsPatchRequest
    ):
        ...
        if "features" in _car_data_dict:
            await self.db.cars_features.update_features_bulk(
                car_id, car_data.features, exclude_unset=True
            )
        await self.db.car_models.edit(
            _car_data, id=car_id, car_mark_name=mark_name, exclude_unset=True
        )
        await self.db.commit()
